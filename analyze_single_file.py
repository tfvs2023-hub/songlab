...
from advanced_vocal_analyzer_no_essentia import AdvancedVocalAnalyzerNoEssentia as AdvancedVocalAnalyzer
...
def analyze_file(file_path):
    ...
    try:
        ...
        print("\n🎤 4축 보컬 분석 결과:")
        print("-" * 40)
        print(f"📊 밝기 (Brightness):    {scores['brightness']:+6.1f} {'🌟 밝음' if scores['brightness'] > 0 else '🌙 어두움'}")
        print(f"📊 두께 (Thickness):     {scores['thickness']:+6.1f} {'🎯 두꺼움' if scores['thickness'] > 0 else '💨 얇음'}")
        print(f"📊 성대내전 (Adduction): {scores['adduction']:+6.1f} {'✅ 완전' if scores['adduction'] > 0 else '⚡ 불완전'}")
        print(f"📊 음압 (SPL):          {scores['spl']:+6.1f} {'💪 강함' if scores['spl'] > 0 else '🍃 약함'}")
        print("-" * 40)
