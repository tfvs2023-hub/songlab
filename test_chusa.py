...
def test_chusa_audio():
    ...
    try:
        ...
        print(
            f"일반인추사닮.wav | {results['brightness']:+6.1f} | {results['thickness']:+6.1f} | {results['adduction']:+7.1f} | {results['spl']:+6.1f} | {results['gender'][:2]} | {results['potential_high_note']}"
        )
        ...
        print(f"\n📍 음압 순위 (높을수록 강함)")
        spl_scores = [("일반인추사닮", spl), ("상일2", 81.8), ("발성만", 78.2)]
