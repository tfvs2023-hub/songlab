...
class VocalAnalyzerLite:
    ...
    def preprocess(self, audio: np.ndarray, sr_orig: int) -> Tuple[np.ndarray, Dict]:
        ...
        if low_freq < high_freq:
            sos = signal.butter(4, [low_freq, high_freq], btype='band', fs=self.sr, output='sos')
    ...
    def extract_features(self, audio: np.ndarray) -> Dict:
        ...
        features['midhi_ratio'] = (features['high_power'] + features['very_high_power']) / total_power
    ...
    def _compute_spectral_tilt(self, power_spec: np.ndarray, freqs: np.ndarray) -> float:
        """스펙트럴 틸트 계산"""
        try:
            # Log scale
    ...
    def compute_scores(self, features: Dict) -> Dict:
        ...
        z_centroid = z_score(features.get('spectral_centroid', 0), *ref_stats['spectral_centroid'])
        ...
        z_h1h2 = z_score(ref_stats['h1_h2'][0] - features.get('h1_h2', 0), *ref_stats['h1_h2'])
        ...
        z_rms_std = z_score(ref_stats['rms_std'][0] - features.get('rms_std', 0), *ref_stats['rms_std'])
        ...
        z_dropout = z_score(ref_stats['pitch_dropout'][0] - features.get('pitch_dropout', 0), 
    ...
    def compute_confidence(self, quality: Dict, features: Dict) -> float:
        ...
        if feature_values:
            feature_cv = np.std(feature_values) / (np.mean(np.abs(feature_values)) + 1e-10)
    ...
    def analyze(self, audio_data: bytes) -> Dict:
        ...
        try:
            ...
            # 품질 게이트
            if quality['snr'] < 15 or quality['clipping_percent'] > 3 or quality['silence_percent'] > 60:
