...
def compare_analysis(file_path):
    ...
    for axis in ['brightness', 'thickness', 'loudness', 'clarity']:
        ...
        print(f"{axis_name:<12} {lite_val:+6.1f}         {studio_val:+6.1f}              {diff:+6.1f}")
    
    print("-" * 70)
    print(f"{'신뢰도':<12} {lite_result['confidence']:6.2f}         {studio_result['confidence']:6.2f}              {studio_result['confidence']-lite_result['confidence']:+6.2f}")
    ...
    print(f"{'LUFS':<20} {lite_quality['lufs']:6.1f}      {studio_quality['lufs']:6.1f}")
    print(f"{'SNR':<20} {lite_quality['snr']:6.1f}      {studio_quality['snr']:6.1f}")
    print(f"{'클리핑 %':<20} {lite_quality['clipping_percent']:6.3f}      {studio_quality['clipping_percent']:6.3f}")
    print(f"{'무음 %':<20} {lite_quality['silence_percent']:6.1f}      {studio_quality['silence_percent']:6.1f}")
