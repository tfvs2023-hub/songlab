...
class PhoneSimulator:
    ...
    def _bandpass_filter(self, audio: np.ndarray, low_freq: int, high_freq: int) -> np.ndarray:
        """대역통과 필터"""
    ...
    def _apply_device_ir(self, audio: np.ndarray, device: str) -> np.ndarray:
        ...
        if device == 'iphone':
            ...
            # 2-4kHz 부스트
            sos_boost = signal.butter(2, [2000, 4000], btype='band', fs=self.sr, output='sos')
    ...
    def _simulate_agc(self, audio: np.ndarray, 
    ...
    def _add_room_reverb(self, audio: np.ndarray, 
                        room_size: float = 0.3,
    ...
    def _add_noise(self, audio: np.ndarray, snr_db: float) -> np.ndarray:
        ...
        if noise_type == 'white':
            ...
            pink = signal.lfilter([0.05, -0.095, 0.05], [1, -2.5, 2.0], 
                                 np.random.normal(0, 1, len(audio)))
            ...
            if random.random() > 0.5:
                ...
                noise[burst_pos:burst_pos+burst_len] += np.random.normal(0, 0.3, burst_len)
