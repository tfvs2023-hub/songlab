...
class AdvancedVocalAnalyzerNoEssentia:
    ...
    def setup_processors(self):
        ...
        self.mel_transform = T.MelSpectrogram(
            sample_rate=self.sr, n_fft=2048, n_mels=128, hop_length=512
        )
    ...
    def _remove_noise(self, audio: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            clean_audio = torch.istft(
                clean_stft, n_fft=2048, hop_length=512, window=torch.hann_window(2048)
            )
        ...
        try:
            ...
            for t in range(magnitude.shape[1]):
                ...
                if mag_frame.sum() > 1e-10:  # 무음이 아닌 경우만
                    centroid = torch.sum(freqs.squeeze() * mag_frame) / torch.sum(
                        mag_frame
                    )
    ...
    def _analyze_thickness_torch(self, audio: np.ndarray) -> float:
        ...
        try:
            ...
            stft = torch.stft(
                audio_tensor, n_fft=2048, hop_length=512, return_complex=True
            )
            magnitude = torch.abs(stft)
            ...
            # 가중 평균
            thickness = (
                0.4 * freq_balance + 0.3 * complexity_score + 0.3 * harmonic_richness
            )
