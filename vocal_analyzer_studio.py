...
class VocalAnalyzerStudio:
    ...
    def _compute_mfcc(self, audio):
        ...
        try:
            ...
            if frames:
                ...
                # Mel scale
                mel_spec = np.dot(self._mel_filterbank(n_mels, n_fft, self.sr), power_spec)
    ...
    def compute_scores(self, features: Dict) -> Dict:
        ...
        brightness = 0.0
        brightness += 40 * np.tanh((features.get('high_power', 0) + features.get('air_power', 0)) * 10)
        brightness += 30 * np.tanh((features.get('spectral_centroid', 1000) - 2000) / 1000)
        brightness += 30 * np.tanh((features.get('f2', 1500) - 2200) / 500)
        
        # 두께: 저주파 에너지 + 낮은 포먼트 + spectral contrast
        thickness = 0.0
        thickness += 40 * np.tanh((features.get('low_power', 0) + features.get('low_mid_power', 0)) * 10)
