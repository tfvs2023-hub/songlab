    ...
    <div className="min-h-screen bg-gradient-to-b from-white to-slate-50 text-slate-900">
      <header className="sticky top-0 z-40 bg-white/70 backdrop-blur border-b border-slate-200">
        <nav className="mx-auto max-w-6xl px-4 sm:px-6 h-16 flex items-center justify-between" aria-label="주 네비게이션">
          ...
          <div className="flex items-center gap-2">
            <button className="inline-flex items-center gap-2 rounded-xl bg-sky-600 px-3 py-2 text-sm font-semibold text-white hover:bg-sky-700" aria-label="무료로 시작">
              무료로 시작
                ...
                {scorePreview.map((s, i) => (
                  <div key={i} className="rounded-xl border p-4">
                    ...
                    <div className="mt-3 h-2 rounded-full bg-slate-100">
                      <div className={`h-2 rounded-full ${s.value >= 0 ? "bg-sky-500" : "bg-slate-400"}`} style={{ width: `${Math.min(Math.abs(s.value), 100)}%` }} />
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section className="py-12">
        <div className="mx-auto max-w-xl px-4 sm:px-6 text-center">
          ...
          <form onSubmit={handleSubscribe} className="mt-4 flex gap-2" aria-label="이메일 구독 폼">
            ...
            <input id="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@email.com" className="flex-1 rounded-xl border px-4 py-3 outline-none focus:ring-2 focus:ring-sky-500" />
            <button type="submit" className="rounded-xl bg-slate-900 text-white px-5 py-3 font-semibold">{subscribed ? "감사합니다" : "구독"}</button>
          </form>
        </div>
      </section>
